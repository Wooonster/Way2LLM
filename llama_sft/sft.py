...
class ModelArguments:
    """
    Arguments for pretraining or finetuning a model.
    """
    ...
    tokenizer_name_or_path: Optional[str] = field(
        default=None,
        metadata={
            "help": "The tokenizer for weights initialization, don't set if train from scratch"
        }
    )
    ...
    low_cpu_mem_usage: bool = field(
        ...
        metadata={
            "help": (
                ...
                "Setting True will benefit LLM loading time and RAM consumption."
            )
        },
    )

    def __post_init__(self):
        # 假设这里有 config_overrides 字段，确保也加上类型注解
        if hasattr(self, "config_overrides") and self.config_overrides is not None and (self.config_name is not None or self.model_name_or_path is not None):
...
class DataTrainingArguments:
    ...
    """
    dataset_dir: Optional[str] = field(default = None, metadata = {'help': 'The name of the dataset to use (via the datasets library).'})
    ...
    validation_split_percentage : Optional[float]  = field(default = 0.05, metadata = {'help': "The percentage of the train set used as validation set in case there's no validation split"})
...
def main():
    ...
    if training_args.do_train:
        with training_args.main_process_first(desc="loading and tokenization"):
            ...
            logger.info(f'path passed in is: {path}')
