...
3. 最后，因为大多数神经网络都是在概率上进行分类，只要数据不影响概率分布，对神经网络的精度影响就不大。

量化神经网络是对每一层而言，对每一层网络进行量化计算，每一层输出时进行反量化。

### 训练后动态量化

1. 将训练好的模型权重量化为 int8，并保存量化参数
2. 在模型推理时，对每一层输入的 fp32 激活值*动态进行量化*，到 int8
3. 在每一层对量化后的 int8 权重 和 int 激活值进行计算
4. 在每一层输出时，将结果反量化为 fp32
5. 将 fp32 激活值传到下一层
